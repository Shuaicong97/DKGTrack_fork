...
class TransRMOT(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, criterion, track_embed,
        ...
        # special tokens
        self.special_tokens = [
            self.tokenizer.cls_token_id,  # [CLS] token ID
            self.tokenizer.sep_token_id,  # [SEP] token ID
    ...
    def forward_text(self, text_queries, motion_map, subject_map, static_map, device):
        ...
        text_sentence_features = encoded_text[0]
        text_sentence_features = self.txt_proj(text_sentence_features)

        sentence_number = text_sentence_features.shape[1]

        motion_map = motion_map[:, :sentence_number].bool()
        subject_map = subject_map[:, :sentence_number].bool()
        static_map = static_map[:, :sentence_number].bool()

        subject_feat = torch.cat([text_sentence_features[subject_map.bool()], text_features], dim=0)
    ...
    def proceesing_text_decoupling(self, text):
        ...
        encodings = encoded_input.encodings[0]
        roberta_offsets = encodings.offsets

        motion_map = torch.zeros(40)
        static_map = torch.zeros(40)
        subject_map = torch.zeros(40)
            ...
            if rt != '<s>' and rt != '</s>' and rt != '.' and rt != '-':
                for token in doc:
                    if token.idx <= start and (token.idx + len(token.text)) >= end: 
                        ...
                        elif token.pos_ == 'ADP' or token.pos_ == 'ADJ':
                            try:
                                static_map[index] = 1
                            except:
                                break
    ...
    def generate_masks_with_special_tokens_and_transfer_map(self, tokenized, special_tokens_list, tokenizer):
        ...
        idxs = torch.nonzero(special_tokens_mask, as_tuple=False)
